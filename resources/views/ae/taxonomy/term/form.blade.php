...
{!! Former::text('name')->label('Term Name')->blockHelp('A name for the term. Must be unique within its vocabulary.') !!}

{!! Former::select('parent_id')->label('Parent Term')->options([
        '' => '_TOP'
    ] + $term->vocabulary->rootTerms()->pluck('name', 'id')->toArray()
)->blockHelp('The root term beneath which to place this one.') !!}

{!! Former::url('url')->label('Term Link')->blockHelp('A URL to which the term should link when displayed.') !!}
