...
<div class="listing-wrapper">
  <nav class="listing-tools">
    <a class="button create-new" href="{{ action(\App\Taxonomy\Category\Application\Controllers\CreateController::class) }}">Create New Category</a>
      ...
      @foreach ($categories as $cat)
        <li id="item-{{ $cat->id }}" class="item">
          <div class="item--header">
            <h3 class="title">
              <a href="{{ action(\App\Taxonomy\Category\Application\Controllers\EditController::class, $cat->id) }}" title="{{ __('Edit') }}">{{ $cat->name }}</a>
            </h3>
          </div>
          ...
          <div class="navigation item--actions">
            <menu>
              <li>
                <a href="{{ action(\App\Taxonomy\Category\Application\Controllers\EditController::class, $cat->id) }}" title="{{ __('Edit article') }}">
              ...
              <li>
                <a href="{{ action(\App\Taxonomy\Category\Application\Controllers\DestroyController::class, $cat->id) }}" onclick="event.preventDefault();document.getElementById('deleteForm').submit();" title="{{ __('Delete article') }}">
                  <i class="fa-solid fa-trash"></i> {{ __('Delete') }}
                </a>
                <form id="deleteForm" class="sr-only" method="POST" action="{{ action(\App\Taxonomy\Category\Application\Controllers\DestroyController::class, $cat->id) }}">
                  @csrf
                  {{ method_field('DELETE') }}
                </form>
              </li>
