        ...
        <project id="item-{{ $project->id }}" class="item">
          ...
          <nav class="navigation item--taxonomy">
            @if (! is_null($project->category))
              ...
              <a itemprop="tag" class="label-category" href="{{ action(\App\Taxonomy\Interface\Http\Controllers\EditController::class, $project->category->id) }}" title="{{ __('Edit category') }}">{{ $project->category->name }}</a>
            @else
              <p class="w-full">
