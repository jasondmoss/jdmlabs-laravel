@php
  use Aenginus\Shared\Enums\Promoted;use Aenginus\Shared\Enums\Status;use Illuminate\Support\Facades\Date;
@endphp
...
<div class="listing-wrapper">
  ...
  <header id="listingHeader" class="listing-header">
    ...
    <nav class="listing-tools">
      <a class="button create-new" href="{{ action(\Aenginus\Client\Interface\Web\Controllers\CreateController::class) }}">Create New Client</a>
      ...
      @foreach ($clients as $client)
        <client id="item-{{ $client->id }}" class="item">
          ...
          <header class="item--header">
            <h3 class="title">
              <a href="{{ action(\Aenginus\Client\Interface\Web\Controllers\EditController::class, $client->id) }}" title="{{ __('Edit') }}">{{ $client->name }}</a>
            </h3>
          </header>

          <p class="item--id"><strong class="label">{{ __('ID') }}:</strong> {{ $client->id }}</p>
          <p class="item--count" style="color: #007741;" class="project">
            <strong class="label">{{ __('Projects') }}:</strong> {{ $client->projects_count }}</p>
          ...
          <aside class="item--meta">
            <span class="status" wire:click="toggleStatePublished('{{ $client->id }}')" title="@if ('published' === $client->status->value) {{ __('Unpublish this client') }} @else {{ __('Publish this client') }} @endif">
              {!! Status::icon($client->status) !!}
            </span>
            <span class="promoted" wire:click="toggleStatePromoted('{{ $client->id }}')" title="@if ('promoted' === $client->promoted->value) {{ __('Unpromote this client') }} @else {{ __('Promote this client') }} @endif">
          ...
          <aside class="item--date">
            ...
            @if (! is_null($client->published_at))
              <time class="published" datetime="{{ Date::parse($client->published_at)->format('c') }}" title="{{ Date::parse($client->published_at)->format('c') }}">
          ...
          <footer class="navigation item--actions">
            <menu>
              <li>
                <a href="{{ action(\Aenginus\Client\Interface\Web\Controllers\EditController::class, $client->id) }}" title="{{ __('Edit client') }}">
                  <i class="fa-solid fa-pen-to-square"></i> {{ __('Edit') }}
                </a>
              </li>
              <li>
                <a rel="external" href="{{ action(\Aenginus\Client\Interface\Web\Controllers\SingleController::class, $client->slug) }}" title="{{ __('View client') }}">
                  <i class="fa-solid fa-eye" style="color: #2ec27e;"></i> {{ __('View') }}
                </a>
              </li>
              <li>
                <a href="{{ action(\Aenginus\Client\Interface\Web\Controllers\DestroyController::class, $client->id) }}" onclick="event.preventDefault();document.getElementById('deleteForm').submit();" title="{{ __('Delete client') }}">
                  <i class="fa-solid fa-trash"></i> {{ __('Delete') }}
                </a>
                <form id="deleteForm" class="sr-only" method="POST" action="{{ action(\Aenginus\Client\Interface\Web\Controllers\DestroyController::class, $client->id) }}">
                  @csrf
                  {{ method_field('DELETE') }}
                </form>
              </li>
