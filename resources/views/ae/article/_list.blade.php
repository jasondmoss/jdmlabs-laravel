        ...
        <article id="item-{{ $article->id }}" class="item">
          ...
          <nav class="navigation item--taxonomy">
            @if (! is_null($article->category))
              <p class=""><i class="fa-solid fa-tag" style="color: #2ec27e;"></i> <a itemprop="tag" href="{{ action(Category\EditController::class, $article->category->slug) }}" title="{{ __('Edit category') }}">{{ $article->category->name }}</a></p>
            @else
              <p class="w-full"><i class="fa-solid fa-tag" style="color: var(--gray-light)"></i> &#160;</p>
            @endif
          </nav>
