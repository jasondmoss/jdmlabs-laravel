...
<x-aenginus.layout title="Edit Article" page=" article" context=" edit" livewire="true">
  ...
  <div class="flex flex-col gap-y-10 p-2 lg:basis-2/3">
    <fieldset class="form-content flex flex-col gap-y-5 px-2 border-t border-gray-300">
      ...
      <div class="flex flex-col gap-y-3">
        <x-shared.trix-editor type="minimal" label="Summary" name="summary" value="{!! $article->summary !!}" placeholder="Write something..."/>
      </div>

      <div class="flex flex-col gap-y-3">
        <x-shared.trix-editor type="full" label="Body" name="body" value="{!! $article->body !!}" placeholder="Write something..."/>
      </div>
    </fieldset>
    ...
    <fieldset class="form-taxonomy flex flex-col gap-y-5 px-2 border-t border-gray-300">
      ...
      <div class="flex flex-col gap-y-3">
        ...
        <select name="category" class="form-select" type="text">
    ...
    <fieldset class="form-image flex flex-col gap-y-5 px-2 border-t border-gray-300">
      ...
      <div class="grid items-start gap-5">
        <div class="sm:col-start-1 sm:col-span-3 flex flex-col gap-y-3">
          ...
          {{ html()->file('signature_image[0][file]')->accept('jpg,png')->attributes([
            'class' => 'file-uploader'
          ]) }}
        </div>
        ...
        <div class="sm:col-start-1 sm:col-span-3 flex flex-col gap-y-3">
          ...
          {{ html()->text('signature_image[0][label]', old('signature_image[0][label]', $article->image->label ?? null)) }}
        </div>

        <div class="sm:col-start-1 sm:col-span-3 flex flex-col gap-y-3">
          {{ html()->label('Alt Description')->for('signature_image[0][alt]')->class('font-medium text-sm') }}
          {{ html()->text('signature_image[0][alt]', old('signature_image[0][alt]', $article->image->alt ?? null)) }}
        </div>

        <div class="sm:col-start-1 sm:col-span-3 flex flex-col gap-y-3">
          {{ html()->label('Caption')->for('signature_image[0][caption]')->class('font-medium text-sm') }}
          {{ html()->text('signature_image[0][caption]', old('signature_image[0][caption]', $article->image->caption ?? null)) }}
        </div>
