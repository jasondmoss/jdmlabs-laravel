      ...
      <table class="flex flex-col gap-y-4 max-w-sm mx-auto mt-10 mb-0 md:table md:max-w-full md:m-0 lg:mt-4 text-left text-sm font-medium">
        ...
        <tbody class="flex flex-col gap-y-10 md:table-row-group w-full">
          @foreach ($categories as $category)
            ...
            <!-- Item -->
            <tr id="item-{{ $category->id }}" class="flex flex-col max-w-full md:table-row border-b odd:bg-white even:bg-slate-50">
              {{--  Name + ID  --}}
              ...
              {{--  Articles Count  --}}
              <td class="block md:table-cell md:align-top md:px-6 py-6">{{ $category->articles_count }}</td>
              {{--  Projects Count  --}}
              <td class="block md:table-cell md:align-top md:px-6 py-6">{{ $category->projects_count }}</td>
              {{--  ACTIONS  --}}
