          ...
          @foreach ($projects as $project)
            ...
            <tr id="item-{{ $project->id }}" class="flex flex-col max-w-full md:table-row border-b odd:bg-white even:bg-slate-50 hover:bg-lime-50">
              ...
              <td class="block md:table-cell md:align-top md:px-6 py-2"><div class="flex flex-col gap-y-2">
                ...
                <p class="flex gap-x-3" title="{{ __('ProjectModel Ulid') }}">
                ...
                >
                  <div class="flex flex-col gap-y-1">
                    <a href="{{ action(Project\DestroyController::class, $project->id) }}"
                        onclick="event.preventDefault();document.getElementById('Delete_{{ $project->id }}').submit();"
                    ...
                    <form id="Delete_{{ $project->id }}" class="sr-only" method="POST" action="{{ action(Project\DestroyController::class, $project->id) }}">
                      @csrf
                      {{ method_field('DELETE') }}
