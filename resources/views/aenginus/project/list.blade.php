              ...
              <td class="block md:table-cell md:align-top md:px-6 py-6"><div class="flex flex-col gap-y-2">
                ...
                @if ($project->category !== null)
                  <p class="flex gap-x-3">
                    ...
                    <a itemprop="tag" class="px-3 py-1 shadow-sm shadow-sky-300 bg-sky-500 hover:bg-sky-600 rounded-sm text-xs text-white" href="{{ action(Taxonomy\EditController::class, $project->category->id) }}" title="{{ __('Edit category') }}">{{ $project->category->name }}</a>
                  </p>
                @endif
              </div></td>
